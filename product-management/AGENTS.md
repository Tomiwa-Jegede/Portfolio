...
## Purpose
The value workspace — what value Victor Portfolio delivers, to whom, proven how, and the status of each value outcome. Value is tracked independently from how engineering delivers it; the delivery plan lives in `delivery-management/` and serves this workspace, not the reverse.
...
- Mark a value outcome delivered only when a stakeholder can feel it — a complete build with no visible proof is a value gap, not a delivery
...
- Re-rank `value-chain-opportunities/MAP.md` when a gap closes or stakeholder weight shifts

## Verification
- Every value-map outcome must reference an owning delivery milestone or be marked an open gap
- Proof measures must be measurable, not aspirational (time, count, route, build result)
- Ensure the root `AGENTS.md` child index references this folder and stays current
- Confirm no unresolved placeholders remain and each Outcome ID is stable and unique

## Child DOX Index

| Path | Owner | Purpose |
|---|---|---|
| `value-map.md` | product-management/ | Canonical value-to-deliver artifact per stakeholder |
| `value-chain-opportunities/MAP.md` | product-management/ | Wayfinder map of unexplored value gaps and opportunities |
